...
{ 
  ...
  mpu.setYGyroOffset(500);//regulate
...
void PWM_calculate(void)          
{
  ...
  K_angle_AD = -7.35;//-7.35(analogRead(K_AGL_AD)-512) * 0.03;
  K_angle_dot_AD = -12.11;//-10.11(analogRead(K_AGL_DOT_AD)-512) * 0.03;
  K_position_AD = -0.31;//-0.31(analogRead(K_POS_AD)-512) * 0.0007;
  K_position_dot_AD = -0.31;//-0.31(analogRead(K_POS_DOT_AD)-512) * 0.0007;
...
void pwm_out(int l_val,int r_val)
{
  ...
  l_val=l_val+5;
  r_val=r_val+20;
  analogWrite(PWM_L, l_val>200? 200:l_val);
  Serial.print(l_val>200? 200:l_val);
  analogWrite(PWM_R, r_val>200? 200:r_val);
  //Serial.print(r_val>255? 255:r_val);
}
